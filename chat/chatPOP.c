...
// Xor function created but not implemented yet.
...
void *reader(void *username){ // Allow custom chat emplacement and name!!
...
// Remake algorithm to make it better & more secure.(fix the lazyness)
char* xor(char* data, char* key){
	int c=0,i;
	for (i = 0; i < sizeof(data); i++)
	{
		data[i]=data[i]^key[c];
		if (c>=sizeof(key) || c+1>=sizeof(key)/sizeof(char))
			c=0;
		else
			c++;
	}
